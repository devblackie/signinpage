...
const Signup = ({ handleLogin }) => {
  ...
  const validateForm = () => {
    ...
    else if (password.length < 4) errors.password = "Password must be more than 4 characters";
    else if (password.length > 10) errors.password = "Password cannot exceed more than 10 characters"; 

    if (!cpassword) errors.cpassword = "Confirm Password is required";
    else if (cpassword !== password) errors.cpassword = "Confirm password and password should be the same";
    return errors;
  };
    ...
    <>
      ...
      {submissionStatus === "success" ? (
        <div className="success">
          ...
          <a onClick={() => navigate("/dashboard")}>Click to continue to the main site</a>
        </div>
      ) : (
        <form onSubmit={handleSignform}>
          ...
          <button className-="bton" type="submit">Signup</button>
        </form>
      )}
    </div>
    </>
